...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $targets = [
            'flow_progress' => ['index' => 'onboarding_flow_progress_unique', 'owner' => 'flow_id'],
            'step_progress' => ['index' => 'onboarding_step_progress_unique', 'owner' => 'step_id'],
        ];
        ...
        foreach ($targets as $key => $target) {
            ...
            /*
             * SQL Server refuses to alter a column an index depends on, and
             * both scope columns sit inside the unique index this migration
             * exists to make effective. So the index comes off, the columns
             * are widened to NOT NULL, and it goes back on with exactly the
             * same name and columns.
             *
             * The drop and the recreate must stay in the same migration: the
             * window between them is the only moment the duplicate guard is
             * absent, and migrations here run inside a transaction.
             */
            Schema::table($table, function (Blueprint $blueprint) use ($target): void {
                $blueprint->dropUnique($target['index']);
            });
            ...
            Schema::table($table, function (Blueprint $blueprint) use ($target): void {
                $blueprint->unique(
                    [$target['owner'], 'subject_type', 'subject_id', 'scope_type', 'scope_id'],
                    $target['index']
                );
            });
        }
