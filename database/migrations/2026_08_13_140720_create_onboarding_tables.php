...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create($this->table('step_progress'), function (Blueprint $table): void {
            $table->uuid('id')->primary();
            /*
             * SQL Server refuses two cascade paths into the same table
             * (error 1785): deleting a flow would reach step_progress both
             * directly through flow_id and indirectly through steps.
             *
             * The step_id cascade below already deletes these rows on the
             * chain flows -> steps -> step_progress, so nothing is orphaned.
             * flow_id therefore stays as an indexed denormalised column
             * without its own cascading constraint. Do not restore the
             * foreign key here without removing the one on step_id.
             */
            $table->uuid('flow_id');
